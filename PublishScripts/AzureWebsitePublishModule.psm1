#  AzureWebSitePublishModule.psm1 は Windows PowerShell スクリプト モジュールです。このモジュールは、Web アプリケーションのライフ サイクル管理を自動化する Windows PowerShell 関数をエクスポートします。この関数をそのまま使用するか、アプリケーションや発行環境用にカスタマイズすることができます。
...
.DESCRIPTION
特定の名前と場所が指定された Windows Azure Web サイトを作成します。この関数は Azure モジュールで New-AzureWebsite コマンドレットを呼び出します。サブスクリプションに名前が指定された Web サイトがない場合、この関数は Web サイトを作成し、Web サイト オブジェクトを返します。このような Web サイトがある場合は、既存の Web サイトを返します。
